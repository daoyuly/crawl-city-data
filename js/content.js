...
Zepto('document').ready(function () {
    ...
    var $ = Zepto;
    var usa_city_data = [];
    var usa_city_list_data = [];

    var mainUrl = 'http://haoweichi.com/Index/cities';
    var cityUrl = 'http://haoweichi.com/Index/get_cities/id/';
    ...
    if (href == mainUrl) {
        ...
        console.log('beigin========进入抓取数据状态');
        usa_city_data.push('');

        $('.city_list li').each(function (i) {
            var a = $(this).find('a').text();
            usa_city_data.push(a);
            console.log(a);
        });

        var s = JSON.stringify(usa_city_data);
        localStorage.setItem('city_list', s);
        // localStorage.setItem('city', s);


        location.href = cityUrl + '1';

    }

    var reg = /(http:\/\/haoweichi.com\/Index\/get_cities\/id\/)([\d]{1,2})/ig;
    ...
    if (match[1] == cityUrl) {
        ...
        var s = localStorage.getItem('city_list');
        var usa_city_data = JSON.parse(s);


        var state = usa_city_data[cityIndex];


        usa_city_data[cityIndex] = {
            state: state,
            city: []
        };
        ...
        $('.city_list li').each(function (i) {
            var a = $(this).find('a').text();
            usa_city_data[cityIndex]['city'].push(a);
            console.log(i + "----" + a);
        });
        ...
        // 不能超过美国州的总数
        if (parseInt(cityIndex) >= 51) {

            console.log('end========数据抓取完成');
            console.log(usa_city_data);

        } else {

            console.log('跳转中...');
            location.href = cityUrl + (parseInt(cityIndex) + 1);
        }
